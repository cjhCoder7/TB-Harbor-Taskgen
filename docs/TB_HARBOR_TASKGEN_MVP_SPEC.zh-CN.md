...
`model.json` 配置 Claude binary、Claude 与 Harbor 外层超时、默认模型、默认 effort 和按 phase 覆盖的 effort：
...
| `scripts/tool_init.sh` | 通过 `uv tool install` 安装 `harbor==0.13.2` 和 `skillnet-ai==0.0.18`。 |
